...
impl Extractor for AsyncExtractor {
    async fn extract(
        ...
        tokio::task::spawn_blocking({
            ...
            move || match archive_type.as_str() {
...
fn extract_zip(archive: &Path, dest: &Path) -> Result<(), ExtractError> {
    let file = std::fs::File::open(archive).map_err(|e| ExtractError::Io(e.to_string()))?;
    let mut zip = zip::ZipArchive::new(file).map_err(|e| ExtractError::Other(e.to_string()))?;
    ...
    for i in 0..zip.len() {
        ...
        if entry.is_dir() {
            std::fs::create_dir_all(&full_path).map_err(|e| ExtractError::Io(e.to_string()))?;
        } else {
            if let Some(parent) = full_path.parent() {
                std::fs::create_dir_all(parent).map_err(|e| ExtractError::Io(e.to_string()))?;
            }
            let mut out =
                std::fs::File::create(&full_path).map_err(|e| ExtractError::Io(e.to_string()))?;
