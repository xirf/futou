...
fn generate_nginx_config(conf_dir: &Path, document_root: &Path) -> Result<(), ProcessError> {
    let root = document_root.display().to_string().replace('\\', "/");
    let conf = format!(
        r#"# Generated by futou
worker_processes 1;
...
impl ProcessManager for WindowsProcessManager {
            ...
            "nginx" => {
                ...
                if !process_alive(pid) {
                    let stderr = child
                        ...
                        .unwrap_or_else(|_| "could not get stderr".into());
                    return Err(ProcessError::ExitError(format!(
                        "Nginx exited immediately. stderr:\n{}",
                        stderr.trim()
                    )));
                }
                Ok(pid)
    ...
    async fn init_data_dir(
        ...
        match runtime {
            ...
            "apache" | "nginx" => {
                let _ = std::fs::create_dir_all(data_dir);
                let _ = std::fs::create_dir_all(data_dir.join("conf"));
                let _ = std::fs::create_dir_all(data_dir.join("logs"));
            }
